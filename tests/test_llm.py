...
def test_openclaw_uses_openai_compatible_client(monkeypatch):
    ...
    assert client._client.chat.completions.last_kwargs["extra_headers"] == {
        "x-openclaw-agent-id": "main"
    }
    assert client._client.chat.completions.last_kwargs["user"].startswith("jarvis-")
...
def test_llm_client_reuses_history_and_session(monkeypatch):
    ...
    assert messages == [
        {"role": "system", "content": client._build_system_prompt([])},
